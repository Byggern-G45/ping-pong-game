...
char mcp_read_rx_buffer(uint8_t buffer, uint8_t startpoint){
	uint8_t address = READ_RX_BUFFER;
...
void mcp_load_tx_buffer(uint8_t buffer, uint8_t startpoint, char data){
	uint8_t address = LOAD_TX_BUFFER;
	address |= (buffer << 2) | (startpoint << 1);
	PORTB &= ~(1<<PD4);
	SPI_Transmit(address);
	SPI_Transmit(data);
